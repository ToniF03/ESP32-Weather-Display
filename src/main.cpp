...
void loop() {
  ...
  if (!client.connect(host, 80))
    failedDataFetch();
  client.print(String("GET /data/2.5/uvi?lat=49.630638&lon=8.357910&units=metric&appid=") + OPENWEATHERMAP_KEY + " HTTP/1.1\r\n" + "Host: " + host + "\r\n" + "Connection: close\r\n\r\n");
  ...
  do {
    ...
    for (float xValue = 389; xValue < 765; xValue += 1) {
      display.drawLine((int)xValue, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue)), (int)xValue + 1, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue + 1)), GxEPD_RED);
      display.drawLine((int)xValue, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue)) - 1, (int)xValue + 1, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue + 1)) - 1, GxEPD_RED);
      display.drawLine((int)xValue, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue)) + 1, (int)xValue + 1, (int)round(Interpolation::ConstrainedSpline(xValues, yValues, numValues, xValue + 1)) + 1, GxEPD_RED);
    }
